...
if (!function_exists("jr_Digg_refresh")) {
function jr_Digg_refresh() {
update_option("jr_submitted_Digg", "0");
}
}
...
update_option("jr_submitted_Digg", "1");
wp_schedule_single_event(time()+172800, 'jr_Digg_refresh'); 
} else {
...
if (get_option("jr_digg_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_digg_link_personal", $content);
}
}
...
$linkper=utf8_decode(get_option('jr_digg_link_personal'));

if (get_option("jr_Digg_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_Digg_links_choice'));
...
update_option("jr_Digg_links_choice", $new);
update_option("jr_Digg_link_newcheck", "444");
}

if (get_option("jr_submitted_Digg")=="0") {
$pname="jr_Digg";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_Digg", "1");
...
wp_schedule_single_event(time()+172800, 'jr_Digg_refresh'); 
} else if (get_option("jr_submitted_Digg")=="") {
$pname="jr_Digg";
$url=get_bloginfo('url');
$current=get_option("jr_Digg_links_choice");
...
wp_schedule_single_event(time()+172800, 'jr_Digg_refresh'); 
  ...
  $pshow = "<p style='font-size:x-small'>Digg Plugin created by ".$linkper." - ".stripslashes(get_option('jr_Digg_links_choice'))."</p>";
  echo $pshow;
}
